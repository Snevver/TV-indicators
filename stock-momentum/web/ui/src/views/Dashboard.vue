...
const t212off = computed(() => store.track === "live" && !h.value?.t212?.configured);
...
<template>
  <div class="page">
    <div class="topbar">
      <div class="seg">
        <button v-for="t in ['paper','live']" :key="t"
                :class="{ on: store.track === t }" @click="setTrack(t)">
          {{ t === 'paper' ? 'Paper' : 'Trading 212' }}
        </button>
      </div>
      <span class="tick tag">
    ...
    <section v-if="t212off" class="note">
      ...
      <p class="lede">{{ h.t212?.reason || "No API key is set." }} Connecting it
        changes nothing about what the bot trades — it only mirrors your real
        positions here.</p>
      <p class="fine"><a href="/settings">Open Settings</a> to add the key.</p>
    </section>

    <template v-else>
      <Hero :s="s" :sym="sym" :series="hist.total"
            :label="store.track === 'live' ? 'Trading 212' : 'Paper'" />
